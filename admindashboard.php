...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    body{margin:0;font-family:Inter,ui-sans-serif,system-ui,-apple-system,"Segoe UI",Roboto,Arial;background:var(--bg);color:#222}
    /* NAV */
    .nav{
      background:#111827;color:#fff;padding:14px 22px;display:flex;align-items:center;justify-content:space-between;
      box-shadow:0 4px 10px rgba(0,0,0,0.06);
    }
    .nav .brand{font-weight:700}
    .nav .links a{color:#fff;text-decoration:none;margin-left:14px;font-size:14px;padding:8px 10px;border-radius:8px}
    .nav .links a:hover{background:rgba(255,255,255,0.06)}
    /* container */
    .wrap{max-width:1200px;margin:24px auto;padding:0 18px}
    /* profile card */
    .profile{
      background:linear-gradient(90deg,#fff9c4,#f1fff7);border-radius:var(--radius);padding:18px;display:flex;align-items:center;justify-content:space-between;
      box-shadow:var(--shadow);border:2px solid #ffe082;
    }
    .profile .info p{margin:6px 0;font-size:15px}
    .profile .actions a{display:inline-block;background:var(--accent);color:#fff;padding:8px 12px;border-radius:8px;text-decoration:none;font-size:14px}
    .profile .actions a:hover{opacity:.95}
    ...
    .grid-4{display:grid;grid-template-columns:repeat(4,1fr);gap:18px;margin-top:18px}
    @media (max-width:1100px){ .grid-4{grid-template-columns:repeat(2,1fr)} }
    @media (max-width:640px){ .grid-4{grid-template-columns:1fr} }

    /* card base */
    .card{background:var(--card);border-radius:var(--radius);padding:16px;height:280px;position:relative;box-shadow:var(--shadow);cursor:pointer;overflow:auto;transition:transform .16s ease, box-shadow .16s ease}
    .card:hover{transform:translateY(-6px);box-shadow:0 22px 50px rgba(17,24,39,0.12)}
    .card .title{font-weight:700;text-align:center;margin-bottom:12px}
    .small-item{background:rgba(255,255,255,0.9);padding:10px;border-radius:10px;margin-bottom:10px;border:1px solid rgba(0,0,0,0.04)}
    .small-item .muted{color:var(--muted);font-size:13px;margin-top:6px}
...
.card.payments{
    ...
    .card .note{position:absolute;left:16px;right:16px;bottom:12px;text-align:center;color:var(--muted);font-size:13px}
    /* stat badge */
    .badge{display:inline-block;background:rgba(0,0,0,0.06);padding:6px 10px;border-radius:20px;font-weight:600;margin-top:8px}

    /* table like layout in manage pages link */
    .manage-link{display:inline-block;margin-top:10px;padding:8px 12px;background:#111827;color:#fff;border-radius:10px;text-decoration:none}
    .manage-link:hover{opacity:.95}

    /* responsive small scroll area */
    .scroll-area{max-height:190px;overflow:auto;padding-right:6px}
    /* footer small */
...
<body>
  ...
  <main class="wrap">
    ...
    <!-- PROFILE -->
    <section class="profile" role="region" aria-label="admin profile">
      <div class="info">
    ...
    <section class="grid-3" aria-label="quick actions">
        ...
        <div class="scroll-area">
          <?php while($t = $turfs->fetch_assoc()): ?>
            <div class="small-item">
              <div style="font-weight:700"><?= e($t['game_name']) ?> <span style="float:right;font-weight:600;color:var(--muted)">ID: <?= e($t['hall_id']) ?></span></div>
              <div class="muted">Price: <?= e($t['price']) ?> Tk</div>
              <?php if(!empty($t['description'])): ?><div class="muted"><?= e(substr($t['description'],0,120)) ?><?= (strlen($t['description'])>120 ? '...' : '') ?></div><?php endif; ?>
            </div>
          <?php endwhile; ?>
        </div>
      ...
      <!-- SLOTS -->
      <a class="card slots" href="manage_slots.php" title="Manage Slots & Bookings">
      ...
      <!-- MANUAL BOOKING -->
      <a class="card manual" href="manual_booking.php" title="Create Manual Booking">
        <div class="title">Manual Booking</div>
        <div style="text-align:center;margin-top:24px;color:var(--muted)">Create a booking on behalf of a customer</div>
        <div class="note">Click to make a manual booking</div>
      </a>
    </section>

    <!-- SECOND ROW: Users | Reviews | Bookings | Payments -->
    <section class="grid-4" aria-label="overviews">
      <!-- USERS -->
      <a class="card users" href="manage_users.php" title="Manage Users">
        <div class="title">Users <span class="badge"><?= e($total_users) ?></span></div>
        <div class="scroll-area">
          <?php while($u = $users->fetch_assoc()): ?>
            <div class="small-item">
              <div style="font-weight:700"><?= e($u['full_name']) ?> <span style="float:right;color:var(--muted)">ID: <?= e($u['customer_id']) ?></span></div>
      ...
      <!-- REVIEWS -->
      <a class="card reviews" href="manage_reviews.php" title="Manage Reviews">
        ...
        <div class="scroll-area">
          <?php while($r = $reviews->fetch_assoc()): ?>
            <div class="small-item">
              ...
              <?php if(!empty($r['comment'])): ?><div class="muted"><?= e(substr($r['comment'],0,100)) ?><?= (strlen($r['comment'])>100 ? '...' : '') ?></div><?php endif; ?>
            </div>
          <?php endwhile; ?>
        </div>
      ...
      <!-- BOOKINGS -->
      <a class="card bookings" href="manage_bookings.php" title="Manage Bookings">
        <div class="title">Bookings <span class="badge"><?= e($total_bookings) ?></span></div>
        <div class="scroll-area">
          <?php while($b = $bookings->fetch_assoc()): ?>
            <div class="small-item">
              <div><strong>Booking #<?= e($b['booking_id']) ?> — <?= e($b['status']) ?></strong></div>
              <div class="muted"><?= e($b['full_name']) ?> • <?= e($b['game_name']) ?></div>
              <div class="muted"><?= e($b['slot_time']) ?> • <?= e($b['booking_date']) ?></div>
            </div>
          <?php endwhile; ?>
        </div>
        <div class="note">Click to view, edit or cancel bookings</div>
      </a>

      <!-- PAYMENTS -->
      <a class="card payments" href="manage_payments.php" title="Manage Payments">
        <div class="title">Payments <span class="badge"><?= e($total_payments) ?></span></div>
        <div class="scroll-area">
          <?php while($p = $payments->fetch_assoc()): ?>
            <div class="small-item">
              <div><strong>Payment #<?= e($p['payment_id']) ?></strong></div>
              <div class="muted"><?= e($p['amount']) ?> Tk • Booking: <?= e($p['booking_id']) ?></div>
              <div class="muted">Status: <?= e($p['status']) ?> • <?= e($p['payment_date']) ?></div>
            </div>
          <?php endwhile; ?>
        </div>
        <div class="note">Click to view or manage payments</div>
      </a>
    </section>

    <div class="footer-note">Dashboard • Quick preview shows recent 3 items per section • Click any card to open full management page</div>
